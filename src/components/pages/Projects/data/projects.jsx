// import weatherApp from "../src/assets/images/weather-dashboard-screen-shot.jpeg";
// import project1 from "../src/assets/images/project-1.jpeg";
// import employeeTracker from "../src/assets/images/employee-tracker-demo.jpeg";
// import techBlog from "../src/assets/images/my-tech-blog.jpeg";
// import project2 from "../src/assets/images/project-2.jpeg";
// import pwa from "../src/assets/images/pwa-text-editor.jpeg";

const projectsData = [
  {
    title: 'Weather App',
    image: '../assets/img/weather-dashboard-screen-shot.jpeg',
    description: 'Description of Weather App',
    link: 'https://slmov215.github.io/weather-dashboard/',
    gitHub: 'https://github.com/slmov215/weather-dashboard',
  },
    {
      title: 'Project 1',
      image: '../assets/img/project-1.jpeg',
      description: 'Description of project 1',
      link: 'https://paulnxn1.github.io/food_Focus/Vegan/veganIndex.html',
      gitHub: 'https://github.com/PaulNXN1/food_Focus',
    },
    {
      title: 'Employee Tracker',
      image: '../assets/img/employee-tracker-demo.jpeg',
      description: 'Description of Employee tracker',
      link: 'https://drive.google.com/file/d/1r7yIBk7enMAX2aziXzwKoqegtvwp2hHs/view',
      gitHub: 'https://github.com/slmov215/employee-traker-sql',
    },
    {
      title: 'My Tech Blog',
      image: '../assets/img/my-tech-blog.jpeg',
      description: 'Description of My Tech Blog',
      link: 'https://my-mvc-tech-blog-slmov-2a0483569b53.herokuapp.com/',
      gitHub: 'https://github.com/slmov215/mvc-tech-blog',
    },
    {
      title: 'Project 2',
      image: '../assets/img/project-2.jpeg',
      description: 'Description of project 2',
      link: 'https://bookbusta-950538aa625f.herokuapp.com/',
      gitHub: 'https://github.com/Nneifem/bookbusta',
    },
    {
      title: 'PWA - JATE',
      image: '../assets/img/pwa-text-editor.jpeg',
      description: 'Description of project 2',
      link: 'https://my-pwa-text-editor-week-19-9be97db16d1d.herokuapp.com/',
      gitHub: 'https://github.com/slmov215/pwa-text-editor',
    },
    // Add more projects
  ];
  
  export default projectsData;
  